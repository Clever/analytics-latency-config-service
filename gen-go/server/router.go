...
import (
	...
	"github.com/Clever/analytics-latency-config-service/gen-go/servertracing"
...
func startLoggingProcessMetrics() {
	metrics.Log("analytics-latency-config-service", 1*time.Minute)
}
...
func newRouter(c Controller) *mux.Router {
	...
	router.Use(servertracing.MuxServerMiddleware("analytics-latency-config-service"))
...
func AttachMiddleware(router *mux.Router, addr string, m []func(http.Handler) http.Handler, options ...func(*serverConfig)) *Server {
	...
	l := logger.New("analytics-latency-config-service")

	handler := withMiddleware("analytics-latency-config-service", router, m, config)
